...
function ControlGroupItem({
  ...
  return (
    <Slot
      ...
      className={cn("rounded-none focus-within:z-10", orientation === "horizontal" &&
        "-me-px h-auto first:rounded-s-md last:-me-0 last:rounded-e-md", orientation === "vertical" &&
        "w-auto [margin-block-end:-1px] first:rounded-ss-md first:rounded-se-md last:rounded-ee-md last:rounded-es-md last:[margin-block-end:0]", className)}
      {...props} />
  );
}
