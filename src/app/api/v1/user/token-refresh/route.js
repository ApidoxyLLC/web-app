...
export async function POST(request) {
    ...
    const newPayload = {  ...(fingerprint   && {     fingerprint         }),
    ...
    const newAccessToken = jwt.sign( {        sub: session._id.toString(),
                                    ...newPayload, 
    ...
    const refreshExpiredPercent = Math.min(100, Math.max(0, Math.floor(((Math.floor(Date.now() / 1000) - payload.iat) / (payload.exp - payload.iat)) * 100)));

    if(refreshExpiredPercent < 80){
        const response = NextResponse.json( {          success: true,
                                                   accessToken: newAccessToken,
                                                  refreshToken: refreshToken,
                                           accessTokenExpireAt: new Date(accessTokenExpiry).toISOString(),
                                          refreshTokenExpireAt: new Date(payload.exp * 1000).toISOString(),
                                                          user: {                                                      
                                                                ...(user.name   && {   name: user.name    }),
                                                                ...(user.email  && {  email: user.email   }),
                                                                ...(user.avatar && { avatar: user.avatar  }),
    ...
    const hashedNewRefreshTokenId = crypto.createHmac('sha256', config.refreshTokenIdHashKey).update(newRefreshTokenId).digest('hex');

    const refreshTokenExpiry = minutesToExpiryTimestamp(refreshTokenExpireMinutes)

     await Session.findByIdAndUpdate( session._id,
                                      {
                                        refreshTokenId: hashedNewRefreshTokenId,
                                        refreshTokenExpiry: refreshTokenExpiry,
