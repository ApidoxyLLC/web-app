...
export async function POST(request) {
  ...
  try {
    ...
    const accessSecret = await decrypt({
    ...
    try {
      ...
      cachedSession = await validateSession({
        vendorId: vendor.id,
        sessionId: accessTokenData.sub,
        tokenId: accessTokenData.tokenId,
      });

      const hashedAccessTokenId = hashTokenId(accessTokenData.tokenId);
      ...
      if (error.name === 'TokenExpiredError' && refreshToken) {
        ...
        if (!safeCompare(session.refreshTokenId, hashTokenId(refreshTokenId))) {
          return clearTokensAndRespond({ error: "Unauthorized..." }, 400);
        }
